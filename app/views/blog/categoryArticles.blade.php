...
<div class="row row-offcanvas row-offcanvas-right" style="display:none;">
      ...
      <div class="col-6 col-sm-6 col-lg-12 panel">
        <h2>
          ...
          <a target="_blank" class="pull-left" 
            ...
            slug) }}"> <i class="glyphicon glyphicon-share" style="font-size:0.5em;margin-right:1em;"></i>
          </a>
        </h2>
        <p>{{ close_tags(Str::limit($article->content_html, 200)) }}</p>
        <p> <i class="glyphicon glyphicon-calendar"></i>
          <span>
            {{ $article->created_at }}（{{ $article->friendly_created_at }}）
          </span>
          <a target="_blank" href="{{ route('blog.show', $article->
            slug) }}#comments">
          ...
          <a href="{{ route('blog.show', $article->
            slug) }}#comments"
                            ...
                            role="button">评论（{{ $article->comments_count }}）
          </a>
        </p>
      </div>
      <!--/span-->
  ...
  <!--/span@include('widgets.blogSidebar', array('activeCategory' =>$category_id))--></div>
<!--/row-->
@stop